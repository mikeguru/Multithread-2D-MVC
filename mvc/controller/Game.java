...
public class Game implements Runnable, KeyListener , MouseMotionListener{
	...
	private void killFoe(Movable movFoe) {
		...
		if (movFoe instanceof Scorable){
			CommandCenter.getInstance().setScore(
					CommandCenter.getInstance().getScore() + ((Scorable) movFoe).score());
		}
	...
	private void spawnNewUfo() {
		...
		if (nTick % 50 == 0) {
			...
			Ufo a = new Ufo();
			//CommandCenter.getInstance().getMovFloaters().enqueue(new NewShipFloater());
			CommandCenter.getInstance().getOpsList().enqueue(a, CollisionOp.Operation.ADD);
			CommandCenter.getInstance().getOpsList().enqueue(new Ufobullet(a), CollisionOp.Operation.ADD);
	...
	public void keyReleased(KeyEvent e) {
		...
		if (fal != null) {
			switch (nKey) {
				...
				case HYPER:
					//set falcon to new location at some random x and y
				...
				case NUM_ENTER:
				//get all foes and detroy them
					for (Movable movable : CommandCenter.getInstance
							().getMovFoes()){
						killFoe(movable);
					}
	...
	public void mouseMoved(MouseEvent e) {
		...
		CommandCenter.getInstance().getOpsList().enqueue(
				new Target(e.getPoint()), CollisionOp.Operation.ADD);

	}


	//in class
	private void scatterDebrics(Movable movable){

		if(movable instanceof Sprite){


            //System.out.println(movable.getRadius()/3);

			for(int nc=0; nc<360;nc+=5){
				CommandCenter.getInstance().getOpsList().enqueue(
